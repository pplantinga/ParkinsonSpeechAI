...
* A recipe for detection of Parkinsons' using `wav2vec2` or `WavLM` to extract features, and `ECAPA-TDNN` as a classification model. The training script can be seen at [train.py](recipes/QPN/detection/train_ecapa.py) and the hyperparameters at [wavlm_ecapa.yaml](recipes/QPN/detection/hparams/wavlm_ecapa.yaml)
