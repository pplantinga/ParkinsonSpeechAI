...
class ParkinsonBrain(sb.core.Brain):
    ...
    def combine_chunks(self, how="avg"):
        ...
        for utt_id, score, label in zip(ids, scores, labels):
...
def dataio_prep(hparams):
    ...
    def transcript_eval_pipeline(transcript):
    ...
    @sb.utils.data_pipeline.takes("transcript")#, "translation")
    @sb.utils.data_pipeline.provides("tokens", "attn_mask")
    def transcript_train_pipeline(transcript):#, translation):
        #print(transcript)
        #print(translation)
    ...
    @sb.utils.data_pipeline.provides("dx_encoded")#, "weight", "to_balance"
    def label_pipeline(dx):
        ...
        """
        yield label_encoder.encode_label_torch(dx)
    ...
    #out_keys = ["id", "tokens", "attn_mask", "patient_type_encoded", "info_dict", "weight", "to_balance"]
    out_keys = ["id", "tokens", "attn_mask", "dx_encoded"]
    for dataset, json_path in train_info.items():
        if dataset == "train":
            dynamic_items = [transcript_train_pipeline, label_pipeline]
        else:
            dynamic_items = [transcript_eval_pipeline, label_pipeline]
    ...
    datasets["train"] = datasets["train"].filtered_sorted(
        key_test={"transcript": lambda x: len(x) > 50},
    )
...
if __name__ == "__main__":
    ...
    print(hparams["transcript_folder"])
