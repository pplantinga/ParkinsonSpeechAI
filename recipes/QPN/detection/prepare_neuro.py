...
def prepare_neuro(
    data_folder, train_annotation, test_annotation, valid_annotation, chunk_size, keep_batch3=False):
    assert os.path.exists(data_folder), "Data folder not found"

    path_type_dict = get_path_type_dicts(data_folder)

    create_json(train_annotation, path_type_dict["train"], chunk_size, overlap=None, keep_batch3=keep_batch3)
    create_json(test_annotation, path_type_dict["test"], chunk_size, keep_batch3=keep_batch3)
    create_json(valid_annotation, path_type_dict["valid"], chunk_size, keep_batch3=keep_batch3)
...
def create_json(json_file, path_type_dict, chunk_size, overlap=None, keep_batch3=False):
    ...
    for audiofile in path_type_dict.keys():
        ...
        if !keep_batch3 and "Batch3" in audiofile:
            continue
